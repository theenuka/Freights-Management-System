...
#    aws s3api create-bucket --bucket freights-tf-state-<YOUR_ACCOUNT_ID> --region us-east-2 --create-bucket-configuration LocationConstraint=us-east-2
...
#    aws dynamodb create-table --table-name freights-tf-lock --attribute-definitions AttributeName=LockID,AttributeType=S --key-schema AttributeName=LockID,KeyType=HASH --billing-mode PAY_PER_REQUEST --region us-east-2

terraform {
  backend "s3" {
    bucket         = "freights-tf-state-837644358356"
